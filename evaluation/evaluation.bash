...
do
    ((count++))
    time_output=$( { time python compression.pyz -i ${testImagesDirectory}/${testImage} -o evalDirectory/${testImage} ; } 2>&1 >/dev/null )
    real_time=$( echo "$time_output" | grep "real" | awk -F"m" '{print $2}' | awk -F"s" '{print $1}')
    total_time=$( echo "$total_time + $real_time" | bc)
    mdice=$(python main.py -i evalDirectory/${testImage} -g ${testGroundTruthImagesDirectory}/${testImage} 2>/dev/null)
